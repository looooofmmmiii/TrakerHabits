...
// habits.php - Improved Habits Manager
// Notes:
// - This file assumes `functions/habit_functions.php` exists and implements
//   the functions listed in the original requirements using prepared statements
//   and proper auth checks.
// - Fallback constants are defined here if not provided by your app config.
// - I focused on security (session, CSRF), bug fixes (timeouts, rate-limit, duplicate functions),
//   and small UX/accessibility improvements.
...
<style>
:root{ --bg:#f6f8fb; --card:#ffffff; --muted:#6b7280; --accent-start:#6366f1; --accent-end:#06b6d4; --danger:#ef4444; --shadow: 0 10px 30px rgba(15,23,42,0.06); --radius:14px; --gap:14px; --container-w:1100px; --text:#0f172a; }
...
.search-row input[type="text"], .search-row select{ height:44px;padding:10px;border-radius:10px;border:1px solid #e6e9ef;background:#fff;font-size:14px; box-shadow: 0 1px 0 rgba(0,0,0,0.02); }
...
.input, textarea, select{ width:100%;padding:12px;border-radius:12px;border:1px solid #e6e9ef;background:#fff;font-size:14px; resize:vertical; }
textarea{min-height:90px}
.btn{ display:inline-flex;align-items:center;justify-content:center;gap:8px; padding:10px 16px;border-radius:12px;border:none;font-weight:700;cursor:pointer; transition:transform .13s ease, box-shadow .12s ease; background:linear-gradient(90deg,var(--accent-start),var(--accent-end)); color:#fff; box-shadow: 0 6px 18px rgba(99,102,241,0.12); }
.btn:hover{transform:translateY(-2px)}
.btn:active{transform:translateY(0)}
.btn-sec{ background:#fff;border:1px solid #e6e9ef;padding:10px 14px;border-radius:12px;font-weight:700;cursor:pointer; }
.btn-danger{ background:var(--danger);color:#fff;padding:10px 14px;border-radius:12px;border:none;font-weight:700;cursor:pointer; }
...
.badge{ display:inline-block;padding:6px 10px;border-radius:999px;font-size:12px;border:1px solid rgba(0,0,0,0.04); background:#f8fafc;color:var(--muted) }
